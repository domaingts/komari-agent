...
func TestParseV2ResponseTreatsHTMLAsProtocolFailure(t *testing.T) {
	...
	if _, err := parseV2Response([]byte("<!DOCTYPE html><html></html>")); err == nil {
...
func TestParseV2ResponseAcceptsResult(t *testing.T) {
	preserveProtocolFallbackState(t)
	response, err := parseV2Response([]byte(`{"jsonrpc":"2.0","id":"r1","result":{}}`))
	if err != nil {
		t.Fatal(err)
	}
	if response.JSONRPC != v2.Version {
		t.Fatalf("unexpected version %q", response.JSONRPC)
	}
}
