...
func buildGPUReport(gpuInfo []unit.DetailedGPUInfo) (gpuReport, bool) {
	if len(gpuInfo) == 0 {
		return gpuReport{}, false
	}
	...
	return gpuReport{
		Count:        len(gpuInfo),
		AverageUsage: totalGPUUsage / float64(len(gpuInfo)),
		DetailedInfo: gpuData,
	}, true
}
...
func GenerateReport() []byte {
	...
	if flags.EnableGPU {
		...
		} else if gpuReport, ok := buildGPUReport(gpuInfo); ok {
			// 成功获取详细信息
			data.GPU = gpuReport
		}
	}
