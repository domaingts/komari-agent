...
func CpuStaticInfo() CpuInfo {
	cpuinfo := CpuInfo{
		...
		CPUCores:         1,
		CPUPhysicalCores: 0, // 0 means unknown and preserves old-server compatibility.
		CPUUsage:         0.0,
	}

	// Prefer gopsutil to avoid lscpu triggering lockdown logs on some kernels.
	info, err := cpu.Info()
	if err == nil && len(info) > 0 {
		modelName := strings.TrimSpace(info[0].ModelName)
		if modelName != "" {
			cpuinfo.CPUName = modelName
		} else if info[0].VendorID != "" || info[0].Family != "" {
